...
public class OnScreenKeyboard : MonoBehaviour
{
    ...
    InputField previousFocus;

    public Transform centerMainKeyboardPoint, leftMainKeyboardPoint;

    [SerializeField] MarkerDisplacer markerDisplacer;
    void Start()
    {
    ...
    public void SetActiveFocus(InputField i)
    {
        ...
        if (focus.text.Equals("0") || focus.text.Equals("0.00"))
        {
            focus.text = "";
            focus.MoveTextStart(true);
        }
        else
        {
    ...
    public void WriteKey(Text t)
    {
        ...
        //focus.onValueChanged.Invoke(focus.text);
    ...
    public void WriteSpecialKey(int n)
    {
        switch (n)
        {
            case 0:
                ...
                {
                    if (focus.text.Length > 1 || focus.contentType != InputField.ContentType.DecimalNumber)
                    {
                        focus.text = focus.text.Substring(0, focus.text.Length - 1);
                    }
                    else
                    {
                        focus.text = 0.ToString();
                    }
                    if (focus.text.Equals("0") || focus.text.Equals("0.00"))
                    {
                        focus.text = "";
    ...
    public void SetActive(bool b)
    {
        if (b)
        {
            if (!isActive)
            {
                gameObject.GetComponent<Animator>().SetBool("Hide", false);
            }
        }
        else
        {
